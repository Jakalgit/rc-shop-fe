...
export function Profile() {
	...
	// Кол-во страниц заказов
	const [orderListTotalPages, setOrderListTotalPages] = useState<number>(1);
	...
	async function getOrdersData() {
		...
		const responseOrders = await getOrdersByProfileId({token: act, page: 1, limit: 12});

		setOrders(responseOrders.records);
		setOrderListTotalPages(responseOrders.totalPages);
	}
	...
	useEffect(() => {
		async function getData() {
			...
			try {
				const act: string = cookies.get("act") || "";
				const responseProfile = await getProfileData(act);
				...
				<section className={styles.orderHistory}>
					<Container>
						...
						{orders.length > 0 ? (
							...
							<div className={`flex items-center justify-center ${styles.emptyOrderWrapper}`}>
								<p>{t("emptyOrders")}</p>
							</div>
						)}
					</Container>
