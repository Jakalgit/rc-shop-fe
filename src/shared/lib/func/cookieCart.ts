...
export function saveCartToCookie(newItem: { article: string; qty: number }) {
	const cartStr = Cookies.get(COOKIE_CART_NAME);
	...
	Cookies.set(COOKIE_CART_NAME, JSON.stringify(cart), {
...
export function getCartFromCookie(): { article: string; qty: number }[] {
	const cookie = Cookies.get(COOKIE_CART_NAME);
	if (!cookie) return [];
	try {
...
export function removeItemFromCartByArticle(article: string) {
	...
	Cookies.set(COOKIE_CART_NAME, JSON.stringify(filteredCart), {
		expires: COOKIE_EXP_DAYS,
		sameSite: 'Lax',
		secure: process.env.NODE_ENV === 'production',
	});
